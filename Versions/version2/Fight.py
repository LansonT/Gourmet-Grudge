...
def fight1(): #controls for player 1
    global player1, player2, x, y, block, blocktime, stun, stun_time, health_check, last_attack_time #health of player2/bot
    ...
    if stun[0] == True: #if player1 is stunned
        while current_time - stun_time[0] < stun_secs * 1000:
            bot()  # This allows the bot to take actions while the player is stunned
            current_time = pygame.time.get_ticks() #update the timer until the while statement is broken
    ...
    #register behavior of input, 1 click = 1 hit
        ...
        if current_time - last_attack_time[0] >= 1 * 1000: #seconds * 1000 milliseconds:
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_w: #'W'
                        ...
                        if block[5] == True:
                            ...
                            player2 -= 1
                            print(f"player 2 = {player2}")
                last_attack_time[0] = current_time #records when player last attack. Should prevent players from spamming
...
def bot():
    ...
    current_time = pygame.time.get_ticks()
    displace = current_time - last_attack_time[1]

    #Stunned
    if stun[1] == True: #If the bot gets stunned
        while current_time - stun_time[1] < stun_secs * 1000:
            current_time = pygame.time.get_ticks()
            pygame.event.get()
    ...
    if block_num > 6:
        while current_time - blocktime < block_secs * 1000:
            #print(f"auto time: {current_time - blocktime} ") #DEBUG PURPOSES
            current_time = pygame.time.get_ticks()
            block[3] = True
            block[4] = True
            block[5] = True
    ...
    if block[block_num] == True:
        if current_time - blocktime >= block_secs * 1000:
        ...
        if displace >= secs * 1000:
            ...
            if number < 3: #for when bot attacks but the player is blocking
                block_num = random.randrange(0, 1, 2)
                if block[block_num] == True:
                    stun[1] = True
                    health_check[1] = player2
                    stun_time[1] = current_time
                    sounds.block.play()
                    print("BLOCKED!")
                    ...
                    if block_num == 0:
                        ...
                        print("Attack Right(3)")
                        player1 -= 1
                        print(f"player 1 = {player1}")
            #blocking
            elif number >= 3 and number <= 6: #bot blocks at the a random direction
                block_num = random.randrange(3, 4, 5) #to block at a random direction and disable the block before the next move
                block[block_num] = True
                print("Bot Block...")
                blocktime = current_time #to record how much time has passed when blocking
            ...
            last_attack_time[1] = current_time